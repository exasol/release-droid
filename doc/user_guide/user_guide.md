...
1. Run Release Droid from a terminal:
   ...
   `java -jar release-droid-1.3.1.jar -name <project name> -goal <goal> -platforms <comma-separated list of platforms>`

   For example:

   `java -jar release-droid-1.3.1.jar -name virtual-schema-common-java -goal validate -platforms github`

   (Optional) Windows: You can simplify this by creating a `release-droid.bat` file containing the following contents `java -jar C:\tools\release-droid-1.3.1.jar %*`.
